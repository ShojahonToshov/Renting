...
export default function Navbar() {
  ...
  return (
    <div>
      <header className="fixed w-full top-0 z-20 bg-gradient-to-b from-emerald-50/90 via-emerald-50/60 to-transparent backdrop-blur-md dark:from-gray-900/90 dark:via-gray-900/60">
        <div className="container mx-auto flex items-center gap-4 p-4 md:px-6">
          {/* Logo + theme toggle */}
          <div className="flex items-center gap-3 title-font font-medium text-gray-900 dark:text-white">
            <Link to="/">
          ...
          <div className="ml-auto hidden items-center gap-4 md:flex">
            ...
            <div className="flex items-center gap-2">
              ...
              {/* Login button — same style as theme toggle */}
              {loged ? (
                <button
                  type="button"
                  onClick={() => {navigate("/admin/dashboard");}}
                  aria-label="Login"
                  className="flex h-10 w-10 items-center justify-center rounded-full bg-white/60 text-gray-500 shadow-sm ring-1 ring-emerald-900/10 backdrop-blur transition-all hover:bg-emerald-100 hover:text-gray-900 hover:ring-emerald-500/30 dark:bg-gray-800/60 dark:text-gray-300 dark:ring-white/10 dark:hover:bg-gray-800 dark:hover:text-white"
                >
                  <User size={18} />
                </button>
              ) : (
      ...
      {/* Sidebar */}
      <aside
        className={`fixed right-0 top-0 z-50 flex h-full w-72 flex-col gap-6 bg-white/80 p-6 shadow-2xl backdrop-blur-xl transition-transform duration-300 md:hidden dark:bg-gray-900/80 ${
      ...
      >
        ...
        <div className="flex items-center gap-1 self-start rounded-full bg-emerald-50/80 p-1 dark:bg-gray-800/80">
          {["uz", "ru", "en"].map((lang) => (
            <button
              key={lang}
              onClick={() => i18n.changeLanguage(lang)}
        ...
        <div className="flex items-center gap-2">
          <button
            type="button"
            onClick={Theme}
            className="flex h-10 w-10 items-center justify-center rounded-full bg-white/60 text-gray-500 shadow-sm ring-1 ring-emerald-900/10 backdrop-blur transition-all hover:bg-emerald-100 hover:text-gray-900 dark:bg-gray-800/60 dark:text-gray-300 dark:ring-white/10 dark:hover:text-white"
          >
            {dark ? <Sun size={18} /> : <Moon size={18} />}
          </button>
        ...
        <Link to="/cart" className="mt-auto" onClick={() => setMenuOpen(false)}>
          <button className="w-full inline-flex items-center justify-center gap-2 rounded-full bg-emerald-500 px-5 py-3 text-sm font-semibold text-white shadow-sm shadow-emerald-500/30 transition-all hover:bg-emerald-600 active:scale-95">
            <ShoppingCart size={16} />
            {t("cart")}
          </button>
        </Link>
